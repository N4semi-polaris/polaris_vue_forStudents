        ...
        <v-row no-gutters>
            <v-col>
                ...
                <v-card color="white" flat width="400px">
                  ...
                  <v-card-title class="justify-center">空き時間、どうする？</v-card-title>
                  ...
                  <div v-show="scheduleDialog_p==1">
                    <v-card-title class="justify-center"><v-btn icon @click="backTo_nextScheduleDialog"><v-icon>mdi-arrow-left-bold-circle</v-icon></v-btn>詳細条件</v-card-title>
                    <v-card-text>
                      <v-col cols="12">
                        <v-checkbox class="nx-2" label="スーパー・コンビニ" color="#210e67"></v-checkbox>
                        <v-checkbox class="nx-2" label="ファッション" color="#032b8d"></v-checkbox>
                        <v-checkbox class="nx-2" label="本・文具" color="#033ba0"></v-checkbox>
                        <v-checkbox class="nx-2" label="映画館" color="#044eb7"></v-checkbox>
                        <v-checkbox class="nx-2" label="飲食店" color="#0461cd"></v-checkbox>
                      </v-col>
                  </v-card-text>
                  </div>
                  <!-- 3ページ目 -->
                    ...
                    <v-card-text><v-row align="center">
                      <v-col cols="12">
                        <v-checkbox
                          ...
                          <v-text-field :disabled="!enabled1"
                          label="交通費（円単位）"></v-text-field>
                      </v-col>
                      <v-col cols="12">
                        <v-checkbox
                          v-model="enabled2"
                          hide-details
                          class="shrink mr-2 mt-0"></v-checkbox>
                          <v-text-field :disabled="!enabled2"
                          label="移動時間（分単位）"></v-text-field>
                      </v-col>
                      <v-col cols="12">
                        <v-checkbox
                          v-model="enabled3"
                          hide-details
                          class="shrink mr-2 mt-0"></v-checkbox>
                          <v-text-field :disabled="!enabled3"
                          label="帰宅時刻（〇〇:〇〇）"></v-text-field>
                      </v-col>
                    </v-row></v-card-text>
                  </div>
                  ...
                  <!-- 以下全ページ共通コンポーネント -->
                  <v-card-actions>
                    <v-btn text color="#0461cd" @click="selectedOpen = false" absolute bottom left>
                      <v-icon left>mdi-close-thick</v-icon>キャンセル</v-btn>
                      <v-btn class="white--text" color="#0461cd" @click="turnTo_nextScheduleDialog" absolute bottom right>次へ
